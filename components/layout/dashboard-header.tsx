...
export function DashboardHeader({ title, subtitle, actions }: DashboardHeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-30 bg-background/85 backdrop-blur-xl border-b border-border px-4 sm:px-6 h-14 sm:h-16 flex items-center justify-between gap-2 sm:gap-4 transition-colors duration-300" style={{ boxShadow: '0 1px 0 0 hsl(var(--border))' }}>
      ...
      <div className="flex items-center gap-1 sm:gap-2 shrink-0">
        ...
        <Link href="/dashboard/notifications">
          <Button variant="ghost" size="icon" className="relative">
            ...
            <span className="absolute top-1.5 right-1.5 h-2 w-2 rounded-full bg-orange-500 shadow-sm shadow-orange-500/60" />
          </Button>
        </Link>
