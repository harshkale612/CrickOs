  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <Link href="/" className="flex items-center gap-3 px-4 h-16 border-b border-(--sidebar-border) shrink-0 hover:opacity-80 transition-opacity">
        <div className="h-8 w-8 rounded-lg bg-linear-to-br from-orange-500 to-orange-600 flex items-center justify-center shadow-lg shadow-orange-500/25 shrink-0">
      ...
      {!collapsed && (
        <div className="px-4 py-3 border-b border-(--sidebar-border)">
          <div className="flex items-center gap-2.5">
            <div className="h-8 w-8 rounded-md bg-linear-to-br from-orange-500 to-orange-700 flex items-center justify-center text-white font-bold text-xs shrink-0">
              MUM
            </div>
            <div className="min-w-0">
              <p className="text-xs font-semibold text-foreground truncate">Mumbai Lions CC</p>
              <p className="text-[10px] text-muted-foreground truncate">MCA Premier Division</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="px-2 py-2 border-t border-(--sidebar-border)">
        ...
        >
          ...
          {notificationCount > 0 && (
            <span className={cn(
              'flex items-center justify-center rounded-full bg-orange-500 text-white text-[10px] font-bold',
      ...
      <div className="px-3 py-3 border-t border-(--sidebar-border)">
        <div className={cn('flex items-center gap-2.5', collapsed && 'justify-center')}>
          ...
          {!collapsed && (
            <button className="text-muted-foreground hover:text-orange-400 transition-colors p-1 rounded" title="Log out">
        ...
        {mobileOpen && (
          <>
            ...
            <motion.aside
              ...
              className="lg:hidden fixed left-0 top-0 bottom-0 w-[min(240px,80vw)] bg-(--sidebar-bg) border-r border-(--sidebar-border) z-50 flex flex-col transition-colors duration-300"
            >
              <button
                onClick={() => setMobileOpen(false)}
